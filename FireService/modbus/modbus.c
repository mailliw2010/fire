...
static int send_msg(modbus_t *ctx, uint8_t *msg, int msg_length)
{
    ...
    memcpy((uint8_t *)UartMesg.sndUartData, msg, (unsigned long)msg_length);
    ...
    do {
        rc = ctx->backend->send(ctx, msg, msg_length);  //send to uart.
        if (rc == -1) {
...
int _modbus_receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
     ...
     * information. */
    step = _STEP_FUNCTION;
    length_to_read = ctx->backend->header_length + 1;   //header_length default is 1
    ...
    while (length_to_read != 0) {
        ...
        memcpy((uint8_t *)UartMesg.recvUartData, msg+msg_length, rc);
...
#if 1       // change to big edian
            dest[i] = (rsp[offset + 2 + (i << 1)] << 8) |
                rsp[offset + 3 + (i << 1)];
#else      // change to little edian
            dest[i] = (rsp[offset + 2 + (i << 1)] << 8) |
                rsp[offset + 3 + (i << 1)];
#endif
...
static int write_single(modbus_t *ctx, int function, int addr, const uint16_t value)
{
    ...
    req_length = ctx->backend->build_request_basis(ctx, function, addr, (int) value, req);    //put data to req
